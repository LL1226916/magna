(function ($, ko, entrance) {
    var DockManagementViewModel = function () {
        ...
        this.startDate = {
            id: "startDate",
            name: "开始日期",
            value: ko.observable(new Date(new Date().toLocaleDateString()).getTime()),
            controlType: "t3",
            readable: true,
            writable: true,
            required: true,
		...
		this.confirmBeforeAction = function(data, type) {
			var that = this;
			if (type === "dock_agree") {		
				that.loading(false);
				that.showConfirm({
                        message:  "确定将状态改为：同意进厂",
						action: "dock_agree",
						data:data
                    }, "confirmBeforeAction");
            } else if(type === "dock_forbit") {
				that.loading(false);
				that.showConfirm({
                        message:  "确定将状态改为：拒绝进厂",
						action: "dock_forbit",
						data:data
                    }, "confirmBeforeAction");
            }
        ...
        this.action = function (data, type) {
			...
			if (type === "dock_agree") {
				that.closePop();							
				request = entrance.service.postTask(data.APPROVAL_TASK_ID_, {
                    "action": "complete",
                    "variables": [
                        {
                            "name": "result",
