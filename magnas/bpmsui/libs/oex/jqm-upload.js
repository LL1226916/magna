        ...
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            if (!$("#uploadItemTemplate").length) {
                var template = "<script type=\"text/html\" id=\"uploadItemTemplate\">" +
                    "<li data-icon=\"delete\">" +
                    "<a href=\"javascript:void(0);\" data-bind=\"event:{click:function(){$root.secureExecute('download',$data)}, taphold:$parent.removeAttachment}\">" +
                    "<i class=\"fas fa-file-import\"></i><span data-bind=\"text:('&nbsp;'+name)\"></span>" +
                    "<p style=\"margin-bottom:0px;\"><font color=\"#78909c\" data-bind=\"text:size\"></font></p>" +
                    "</a>" +
                    "</li>" +
                    "</script>";
            ...
            var currentValue = ko.unwrap(viewModel.value);
            if (!currentValue) {
                currentValue = [];
            ...
            viewModel.attachments = ko.observableArray(currentValue);
