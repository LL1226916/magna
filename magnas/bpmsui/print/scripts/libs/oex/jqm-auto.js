...
ko.bindingHandlers[getBindingName("auto")] = (function () {
    ...
    return {
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            var dataSource = null;
            var meta = null;
            for (var i in entrance.config.autoDataSource) {
                ...
                if (elementName.indexOf(sourceName) >= 0) {
                    dataSource = entrance.config.autoDataSource[i];
                }
            }
            if (root.getAuto) {
    ...
    var getAuto = function (url, keyword, userId) {
        ...
        return $.ajax(settings);
                ...
                if (dataSource || root.getAuto) {
                    var fn = root.getAuto || getAuto;
                    fn(dataSource, text.trim(), "").then(function (result) {
                    ...
                    entrance.Services.getDependency(
                        {
                            collection: meta,
                            tenantId: "",
                            page: 1,
